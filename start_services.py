...
def start_local_ai():
    ...
    # Check if override file exists (for proxy configuration and custom settings)
    override_compose_path = "docker-compose.override.yml"
    if os.path.exists(override_compose_path):
        print("Found docker-compose.override.yml, applying overrides...")
        compose_files.extend(["-f", override_compose_path])
